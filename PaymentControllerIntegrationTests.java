...
public class PaymentControllerIntegrationTests {
    ...
    public void testRemoveUser() throws Exception {
        ...
        doReturn(true).when(repository).removeUser(sender.getEmail());
        doReturn(false).when(repository).removeUser("dummy");

        mockMvc.perform(MockMvcRequestBuilders.delete(endpoint, sender.getEmail()) //Ce fel de metoda sa ruleze, in cazul acesta DELETE
                .header((Credentials.AUTHORIZATION), credentials("dummy", "dummy"))) // Header-ul cu valoarea
                .andExpect(MockMvcResultMatchers.status().isUnauthorized()); // E unath pt ca user ul dummy cu password-ul dummy nu exista in baza
    ...
    public void testInsertAccount() throws Exception {
        ...
        doReturn(true).when(repository).saveAccount(senderAccounts.get(0));

        mockMvc.perform(MockMvcRequestBuilders.put(endpoint)
                .header((Credentials.AUTHORIZATION), credentials("dummy", "dummy")) //DAca nu are credentialele bune da eroare
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(sender)))
                .andExpect(MockMvcResultMatchers.status().isUnauthorized());
    ...
    public void testRemoveAccount() throws Exception {
        ...
        doReturn(true).when(repository).removeAccount(sender.getId(), senderAccounts.get(0).getCurrency());
        doReturn(false).when(repository).removeAccount("9999", AccountModel.Currency.EUR); //Returnam false daca nu exista in baza

        mockMvc.perform(MockMvcRequestBuilders.delete(String.format(endpoint, sender.getEmail(), (senderAccounts.get(0)).getCurrency()))
                .header((Credentials.AUTHORIZATION), credentials("Dummy", "Dummy")))
                .andExpect(MockMvcResultMatchers.status().isUnauthorized());
        mockMvc.perform(MockMvcRequestBuilders.delete(String.format(endpoint, sender.getEmail(), (senderAccounts.get(0)).getCurrency()))
                .header((Credentials.AUTHORIZATION), credentials(adminEmail, adminPassword)))
